...
PC 向けの通常クライアントは、Flet を使ったデスクトップアプリとして別サブプロジェクト化する。
Flet クライアントは、ブラウザ検証用 UI とは分けて、サーバー接続、字幕表示、設定 UI、将来のマイク入力を扱う主クライアント候補とする。
...
- Client Flet: Python, Flet, websockets
...
- `requirements-flet.txt`: Flet クライアントの依存関係
...
- `docs/flet_client.md`: Flet クライアントの設計とセットアップ
...
- `client/flet/`: Flet クライアントの最小 smoke test UI。実装済み。
...
- `npm run dev:flet`: Flet クライアントをデスクトップアプリとして起動するスクリプト。実装済み。
