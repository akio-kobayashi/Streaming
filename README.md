...
PC 向けの通常クライアントは、Qt を使ったデスクトップアプリとして別サブプロジェクト化する。
Qt クライアントは、ブラウザ検証用 UI とは分けて、サーバー接続、PC マイク入力、字幕表示、設定 UI、リアルタイムスペクトログラムを扱う主クライアント候補とする。
Flet クライアントは高速描画と音声入力 UI の再現性に問題があるため、実験扱いに降格する。
...
- PC desktop client: Qt client
- Development client: Python CLI client
- Experimental desktop client: Flet client
...
Quest 3 連携は、パススルー映像を生かした状態で画面下部に放送字幕風の字幕を重ねることを目標にする。
まず Meta Quest Browser でブラウザクライアントを動かし、音声入力、WebSocket 接続、字幕描画、設定 UI を検証する。
その後、パススルー上の字幕オーバーレイを WebXR で実装できるか実機確認し、必要なら Unity / Meta Quest ネイティブアプリに進む。

Quest Browser は初期検証用の正しい選択肢とする。
理由は、インストールなしで WebSocket、Web Audio API、字幕 UI、設定 UI を検証でき、サーバー側の ASR プロトコルを早く固められるためである。
ただし、最終的な常用クライアントとして正しいかは別問題であり、パススルー上の安定した字幕表示、権限、長時間動作、起動導線に制約がある場合は Unity / ネイティブ Quest アプリへ移行する。
...
字幕は Quest 3 側で受け取り、パススルー表示を妨げないよう画面下部に重ねる。
...
最終仕様では、パススルー映像を維持したまま、画面下部に字幕だけを重ねる。
通常ブラウザ表示の検証段階では、2D ページとして以下を表示する。
...
発話ごとのスペクトログラムや詳細ログは、Quest 3 の本番字幕画面では常時表示しない。
設定パネルまたはデバッグ表示として折りたたみ、通常時はパススルー映像と字幕の視認性を優先する。

### パススルー字幕モード

Quest 3 の主仕様は、パススルー映像の上に字幕オーバーレイを表示する `quest_passthrough_caption` モードとする。
このモードでは、Quest 3 は音声入力端末であり、同時にサーバーから返る字幕を表示するクライアントになる。
...
- デフォルトは放送字幕風の見た目にする。
- 確定部分と未確定部分を色分けする。
- 新しい字幕は下側に入り、確定字幕はスクロールアップで押し上げる。
- 古い字幕は表示行数を超えたら上から消す。
- 設定 UI は通常隠し、必要な時だけ開く。

実装ルート:

1. 通常ブラウザ字幕モードで、音声入力、WebSocket、字幕描画、設定 UI を完成させる。
2. Quest 3 Browser で WebXR / パススルー表示とマイク入力を実機確認する。
3. WebXR で十分なら、字幕パネルをパススルー空間の画面下部に固定する。
4. WebXR で制約が大きい場合は、同じ WebSocket プロトコルを使う Unity / ネイティブ Quest アプリを追加する。

判断基準:

- Quest Browser で十分: 研究デモ、短時間評価、設定変更の多い実験、インストール不要の利用。
- Unity / ネイティブが必要: 常用、長時間運用、確実なパススルー重畳、起動時から字幕だけを出したい場合、配布や権限管理が必要な場合。
...
字幕オーバーレイのデフォルトは放送字幕風にする。
背景は半透明の黒、文字は白、未確定文字は薄い灰色、表示位置は下部、表示行数は 2 行、スクロール方式は `push_up` とする。
...
caption:
  position: bottom
  mode: quest_passthrough_caption
...
WebXR 対応は第 2 段階とする。
WebXR を使う場合は、パススルー映像を維持したまま字幕パネルを画面下部または視野下部に固定表示し、コントローラまたはハンドトラッキングで言語選択や録音開始を行う。
...
5. パススルーを維持した字幕表示を WebXR で実機検証する。
6. WebXR で実現できる場合は WebXR 字幕パネルを追加する。
7. WebXR に制約がある場合のみ、Unity またはネイティブアプリ化を検討する。

### Quest 3 サブプロジェクト管理

Quest 3 対応は大きくなる可能性が高いため、最初から境界を分けて管理する。
ただし、WebSocket プロトコル、字幕メッセージ形式、字幕設定、サーバー接続設定は ASR システム全体と共有するため、当面は同一リポジトリ内のサブプロジェクトとして管理する。

管理方針:

- サーバー、CLI、通常 Web クライアント、Quest 3 クライアントを同一リポジトリで管理する。
- Quest 3 固有の UI、パススルー表示、WebXR/Unity 実装は専用ディレクトリに分ける。
- 共有する通信プロトコルと設定スキーマは `shared/` または `docs/protocol.md` に切り出す。
- Quest 3 側はサーバー実装に依存せず、WebSocket API のみを契約として使う。
- Quest 3 実機検証メモ、既知の制約、ビルド手順を `docs/quest3.md` に集約する。

推奨ディレクトリ構成:

```text
server/                 ASR server, VAD, Whisper integration
client/
  cli/                  WAV and microphone test client
  qt/                   Qt-based PC desktop client
  flet/                 experimental Flet prototype
  web/                  shared browser client
  quest-web/            Quest Browser / WebXR client
shared/
  protocol/             message schemas and examples
docs/
  protocol.md           WebSocket protocol
  quest3.md             Quest 3 setup, constraints, test notes
unity-quest/            optional future Unity / native Quest app
```

段階ごとの管理:

1. `client/web` で通常ブラウザ版の通信、録音、字幕表示を完成させる。
2. `client/quest-web` に Quest 3 用レイアウトとパススルー字幕モードを分離する。
3. WebXR で十分なら `client/quest-web` を継続して育てる。
4. Unity / ネイティブ化が必要になった場合は `unity-quest/` を追加し、通信プロトコルは `shared/` の仕様に合わせる。
5. Quest 3 側が独立アプリとして大きくなり、リリースサイクルや依存関係が完全に分かれた時点で、別リポジトリ化を検討する。

別リポジトリ化の判断基準:

- Unity プロジェクトや Meta SDK 依存が大きく、Web/サーバー開発とビルド環境が衝突する。
- Quest 3 アプリ単体で配布、バージョン管理、リリース作業が必要になる。
- サーバーとは別チームまたは別端末で継続開発する。
- リポジトリサイズが大きくなり、通常 ASR サーバー開発に支障が出る。
...
Client
  ...
  - Quest 3 passthrough caption overlay
...
- Client Qt: Python, PySide6, numpy, sounddevice, websockets
- Client Flet: Python, Flet, websockets, experimental only
...
- ngrok などの HTTPS/WSS トンネルを使う一時検証手順を用意する。
...
Quest 3 用サブプロジェクトを検証する場合:

```bash
npm run dev:quest
```

Quest 3 Browser から `https://<client-host>:5174` 相当の HTTPS 配信 URL を開く。
初期の静的配信は `http://<client-host>:5174` でも確認できるが、マイク入力と実機運用では HTTPS/WSS を使う。
...
ngrok などのトンネルサービスを使って、Quest 3 や別ネットワーク上のクライアントからサーバーへアクセスさせることも可能である。
特に HTTPS/WSS の証明書設定を一時的に簡略化したい場合や、同一 LAN に置けない場合に有効である。

接続イメージ:

```text
Quest 3 Browser
  https://<public-client-url>
  wss://<public-server-url>/ws

Tunnel
  public HTTPS/WSS endpoint

Local / GPU ASR Server
  http://127.0.0.1:8000
...
- 音声ストリーミングのため、トンネル経由では遅延とジッタが増える可能性がある。
- 公開 URL になるため、検証時でも推測されにくい URL、認証、アクセス制限を検討する。
- 無料プランや一時 URL では URL が変わる場合がある。
- サーバー側ログに接続元、セッション ID、音声受信量を残し、想定外アクセスを検出できるようにする。
- 本番運用では、固定ドメイン、TLS、認証、レート制限を備えたリバースプロキシを検討する。
...
- `requirements-qt.txt`: Qt クライアントの依存関係
- `requirements-flet.txt`: Flet 実験クライアントの依存関係
- `package.json`: Web クライアントの依存関係と起動スクリプト
- `config.example.yaml`: 編集用の設定例
- `.env.example`: 必要な環境変数の例
- `run_flet_client.bat`: Windows 用 Flet クライアントのダブルクリック起動ランチャー
- `assets/app-icon.svg`: Windows/Flet クライアント用アイコンの元データ
- `README.md`: インストール、起動、Quest 3 接続手順
- `docs/quest3.md`: Quest 3 サブプロジェクトのセットアップと実機検証メモ
- `docs/qt_client.md`: Qt クライアントの設計とセットアップ
- `docs/flet_client.md`: Flet 実験クライアントの記録
- `docs/windows_client.md`: Windows デスクトップクライアントの配布手順
- `docs/testing.md`: サーバー、CLI、ブラウザ、Quest 3 のテスト手順
...
- `client/qt/`: Qt クライアントの PC マイク送信 UI、レベルメータ、スペクトログラム、字幕表示。初期実装済み。
- `client/flet/`: Flet 実験クライアント。主開発対象から外す。
- `run_flet_client.bat`: Windows でダブルクリック起動するランチャー。実装済み。
...
- `npm run dev:qt`: macOS の Qt 専用 venv から Qt クライアントを起動するスクリプト。初期実装済み。
- `scripts/macos/build_qt_app.sh`: macOS 用 Qt `.app` を生成するスクリプト。初期実装済み。
- `scripts/macos/build_qt_dmg.sh`: 配布用の macOS `.dmg` を生成するスクリプト。
- `npm run dev:flet`: Flet 実験クライアントを起動するスクリプト。
- `npm run dev:flet:web`: Flet クライアントを補助的に Web 表示で起動するスクリプト。実装済み。
