...
class StreamingQtClient(QtWidgets.QMainWindow):
    def __init__(self, server_url: str, insecure_tls: bool = False) -> None:
        ...
        self.latency_send_timer = QtCore.QTimer(self)
        self.latency_send_timer.setSingleShot(True)
        self.latency_send_timer.setInterval(200)
    ...
    def connect_signals(self) -> None:
        ...
        self.language.currentTextChanged.connect(self.queue_language_config)
        self.latency_ms.valueChanged.connect(self.queue_latency_config)
        self.latency_send_timer.timeout.connect(self.flush_latency_config)
    ...
    def connect_server(self) -> None:
        ...
        with self.config_lock:
            self.current_language = self.language.currentText()
            self.current_latency_ms = self.latency_ms.value()
        url = self.server_url.text().strip()
        insecure_tls = self.insecure_tls.isChecked()
        self.worker = threading.Thread(target=lambda: asyncio.run(self.client_loop(url, insecure_tls)), daemon=True)
        self.worker.start()

    async def client_loop(self, url: str, insecure_tls: bool) -> None:
        ...
        try:
            async with websockets.connect(
                ...
                max_size=None,
                open_timeout=10,
                ssl=self.ssl_context(url, insecure_tls),
            ) as websocket:
    ...
    def ssl_context(self, url: str, insecure_tls: bool) -> ssl.SSLContext | None:
    ...
    def send_json(self, data: dict[str, Any]) -> None:
        if self.loop and self.websocket:
            future = asyncio.run_coroutine_threadsafe(self.websocket.send(json.dumps(data)), self.loop)
            future.add_done_callback(self.log_send_error)

    def log_send_error(self, future: asyncio.Future) -> None:
        try:
            future.result()
        except Exception as exc:  # noqa: BLE001
            self.bridge.log.emit(f"send error: {type(exc).__name__}: {exc}")

    def queue_language_config(self, language: str) -> None:
        with self.config_lock:
            self.current_language = language
        self.send_json({"type": "config", "language": language, "language_apply": "next_utterance"})

    def queue_latency_config(self, value: int) -> None:
        with self.config_lock:
            self.current_latency_ms = value
        self.latency_send_timer.start()

    def flush_latency_config(self) -> None:
        with self.config_lock:
            latency_ms = self.current_latency_ms
        self.send_json({"type": "config", "latency_ms": latency_ms})

    def start_recording(self) -> None:
        if self.audio_stream:
            return
        if not self.websocket or not self.loop or not self.audio_queue:
            self.append_log("recording error: websocket is not connected")
    ...
    def stop_recording(self) -> None:
        ...
        if self.audio_stream:
            ...
            self.audio_stream = None
        self.clear_level_meter()
        QtCore.QTimer.singleShot(100, self.clear_level_meter)
        self.record_button.setEnabled(self.websocket is not None)
        self.stop_button.setEnabled(self.websocket is not None)
        self.set_status("stopped")
    ...
    def clear_level_meter(self) -> None:
    ...
    def run_self_test(self) -> None:
        ...
        self.latency_ms.setValue(1200)
        self.latency_ms.setValue(800)
